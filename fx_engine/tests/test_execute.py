...
async def test_execute_debits_source_and_credits_destination(client, funded_customer, pending_quote):
    # Get balances before.
    before = await client.get(f"/customers/{funded_customer['id']}/balances")
    before_usd = next(
        (Decimal(b["amount"]) for b in before.json()["balances"] if b["currency"] == "USD"),
        Decimal("0"),
    )
...
async def test_execute_fails_on_wrong_customer(client, client2=None):
    ...
    c1 = (await client.post("/customers", json={"name": "C1", "email": "c1@test.example"})).json()
    c2 = (await client.post("/customers", json={"name": "C2", "email": "c2@test.example"})).json()
    ...
    quote = (await client.post(
        "/quotes",
        json={"customer_id": c1["id"], "from_currency": "USD", "to_currency": "EUR", "amount": "50.00"},
    )).json()
...
async def test_execute_atomicity_second_leg_negative_balance_rolls_back(client, customer):
    """
